  ...
  it('POST /auth/signup - User SignUp Validation Test(Required)', (done) => {
    chai
      ...
      .then((res) => {
        ...
        assert.equal(res.body.error.f_name, 'First name with minimum of 2 characters long is required');
      });
    done();
  });
  it('POST /auth/signup - User SignUp Validation Test(firstName, lastName)', (done) => {
    chai
      ...
      .then((res) => {
        ...
        assert.equal(res.body.status, 400);
        assert.equal(res.body.error.f_name, 'First name with minimum of 2 characters long is required');
        assert.equal(res.body.error.l_name, 'Last name with minimum of 2 characters long is required');
      });
    done();
  });
  ...
  it('POST /auth/signup - User SignUp successfully', (done) => {
    chai
      ...
      .then((res) => {
        ...
        assert.equal(res.body.data.f_name, 'kim');
        assert.equal(res.body.data.l_name, 'shawn');
      });
    done();
  });
});
  ...
  it('POST /auth/signin - User SignIn success', (done) => {
    chai
      ...
      .then((res) => {
        ...
        assert.equal(res.body.data.f_name, 'joel');
        assert.equal(res.body.data.l_name, 'ugwumadu');
      });
    done();
  });
